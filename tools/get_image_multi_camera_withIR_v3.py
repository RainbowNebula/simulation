...
def configure_cameras():
    ...
    for dev in devices:
        ...
        for sensor in dev.query_sensors():
            if sensor.get_info(rs.camera_info.name) == 'Stereo Module':
                ...
                print(f"设备：{dev.get_info(rs.camera_info.serial_number)} Stereo baseline: {baseline:.3f} mm")
    ...
    return pipelines, configs, serial_numbers, baselines
...
def setup_directories(serial_numbers):
    ...
    now = time.localtime()
    time_dir = time.strftime("%Y%m%d-%H%M-%S", now)
...
def main():
    pipelines, configs, serial_numbers, baselines = configure_cameras()
    directories = setup_directories(serial_numbers)

    print(f"已读取{len(serial_numbers)}个相机")
    ...
    for idx, (serial, profile) in enumerate(zip(serial_numbers, profiles)):
        # 获取 RGB 内参
